...
export default function Home() {
  ...
  const [cacheStored, setCacheStored] = useState(false);
      ...
      while (true) {
        ...
        for (const { event, data } of events) {
          ...
          if (event === 'done') {
            ...
            setAlignmentLoading(false);
            setFromCache(false);
            setCacheStored(d.cacheStored === true);
            setIsComplete(true);
              ...
              {isComplete && (
                <>
                  ...
                  <p style={{ margin: '14px 0 0', fontSize: 11, color: 'rgba(200,180,150,.4)', lineHeight: 1.5 }}>
                    {fromCache
                      ? 'Served from community cache - no API cost.'
                      : cacheStored
                        ? 'Fetched live and permanently cached.'
                        : 'Fetched live; cache storage is currently unavailable.'}
